...
class Dataset(Dataset):
    def __init__(self, root_dir, is_train, apply_flip=False):
        ...
        self.load_volumes(os.path.join(root_dir, folder))
    ...
    def load_images(self, path):
        ...
        for vid in video_folders:
            ...
            for i in range(len(full_paths) - self.seq_len):
                self.samples.append({
                    'vid': vid,
                    'frame_id': int(files[i+self.seq_len].split('.')[0]),
                    'input': full_paths[i : i+self.seq_len],
                    'target': full_paths[i+self.seq_len]
                })

    def load_volumes(self, path):
        files = sorted(glob.glob(os.path.join(path, '*.mat')))
        for f in tqdm(files, desc="Loading Volumes"):
            # Extract ids
            vid = os.path.basename(f).replace('vol', '').replace('.mat', '')
            
            mat = scipy.io.loadmat(f)
            # Transpose to take frames as first dim
            data = mat['vol'].transpose(2, 0, 1) / 255.0
            self.cache[vid] = data.astype(np.float32)
    ...
    def process_data(self, data):
        ...
        img = cv2.resize(data, (CONFIG['IMG_SIZE'], CONFIG['IMG_SIZE']))
    ...
    def __getitem__(self, idx):
        ...
        vol_data = self.cache[item['vid']]
        start = item['input_idx']
        
        input_slice = vol_data[start : start + self.seq_len]
        target_slice = vol_data[start + self.seq_len]
        
        inputs = torch.stack([self.process_data(f) for f in input_slice])
        target = self.process_data(target_slice)

        if self.is_train:
            return inputs, target
        else:
            return inputs, target, int(item['vid']), int(item['frame_id'])
...
def inference(model):
    ...
    filename = f"submission_vol_flip_lstm.csv"
